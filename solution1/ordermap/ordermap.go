...
// Unmarshal和set的时候填充keysIndex，delete, SortKeys时重新填充keysIndex
// 增加replace方法，用于替换某个key的值, 同时要填充value, 删除原有value
// 要考虑递归场景,主要是unmarshal的时候
func (o *OrderedMap) ReplaceKey(old, new string) {
	// check key is in use
	_, ok := o.values[old]
	if !ok {
		return
	}
	index, ok := o.keysIndex[old]
	if !ok {
		return
	}
	value := o.values[old]
		...
		if delim, ok := token.(json.Delim); ok {
			...
			case '{':
				if values, ok := o.values[key].(map[string]interface{}); ok {
					...
					o.values[key] = newMap
				} else if err = decodeOrderedMap(dec, &OrderedMap{keysIndex: make(map[string]int)}); err != nil {
					return err
				}
			case '[':
